node {
    ...
    stage('Build image') {
       ...
       // bat 'docker pull aquasec/trivy:0.19.2'
        // bat 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v "%cd%":/root/.cache/ aquasec/trivy:0.18.3 image --script custom-policy.rego khushiporwal/sample-app:%BUILD_NUMBER%'
        // bat 'docker run --rm -v "%cd%":/root/.cache/ aquasec/trivy:0.19.2 image khushiporwal/sample-app:%BUILD_NUMBER%'
        bat 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock custom-trivy:0.18.3 image khushiporwal/sample-app:%BUILD_NUMBER%'
